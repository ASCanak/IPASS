...
int main(){
    ...
    clockData time; // use the clockData struct format {hours, minutes, seconds, dayoftheWeek, date, month, year} or use the format noted below
    // time.hours = 0; time.minutes = 0; time.seconds = 0; // HH::MM::SS
    // time.day = 1; time.date = 1; time.month = 1; time.year = 21; // DOW DD/MM/YY
    clock.write(time);
    ...
    for(;;){
        if(!black_Switch.read() && (state != idle_State || state != pauze_State)){ //Increases the time depending on the state
        ...
        if(!white_Switch.read() && state != idle_State){ //Decreases the time depending on the state
            if(state == hours_State){
                if(time.hours != 0)
                    time.hours   -= 1;
                else 
        ...
        if(!green_Switch.read() && state != pauze_State){ //Changes the state depending on the state
            if(state != year_State)
                state += 1;
            else
                state = idle_State;
        ...
        if(!red_Switch.read()){ //Pauzes or resumes the clock
            if(time.seconds >= 80){
                clock.resume();
                state = idle_State;
            }
        ...
        if(clock.read(time) && state != pauze_State){ //Draws the clock if clock is available
            upper_Screen << "\f";
